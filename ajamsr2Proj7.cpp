...
class ArchipelagoExpedition
{
 ...
 // mark everyone back to unvisited
 // ...
 ...
 // for doShortestPath
 // ...
 bool bfs(int b, MyQueue* queueList) {
     while (!queueList->isEmpty()) {
         int a = queueList->atTheFront();
         queueList->remove();
         
         MyNode *temp = islandList[a]._getHead();
         for(; temp != NULL; temp=temp->getNext()) {
             if ((islandList[temp->getElem()].isVisited() == false) ||
                 (islandList[temp->getElem()].getPosition() == -1) ) {
                 islandList[temp->getElem()].markVisited();
                 islandList[temp->getElem()].changePosition(a);
             
                 if(temp->getElem() == b) {
                     return true;
                 }
                 
                 queueList->insert(temp->getElem());
             }
         }
     }
     return false;
 ...
 MyList *breadthFirstSearch(int x, int y)
 {
     markBackToUnvisited();
     for(int i=0; i < size+1; i++) {
         islandList[i].changePosition(-1);
     }
     
     MyQueue *IslandQueue = new MyQueue();
     ...
     if( bfs(y, IslandQueue) == false) {
         ...
         printf("You can get from island %d to island %d in one or more ferry rides.\n", x,y);
         MyStack *PathList = new MyStack();
         int currentIsland = y;
         PathList->insert(y);
         do{
             currentIsland = islandList[currentIsland].getPosition();
 ...
 void doShortestPath()
 {
   ...
   if(!path->isEmpty()) {
       printf("Printing the shortest path: ");
       path->show();
   }
   printf("\n");
 }
